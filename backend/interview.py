...
# --- Helper Functions ---
def update_probability(suspect, evidence, lie_probability, game, killer_clue=None, weapon_clue=None, suspect_name=None):
    ...
    if killer_clue and killer_clue in get_suspect_clues(suspect):
        clue_boost += 0.1
    if weapon_clue and weapon_clue in get_weapon_clues(game['killer_weapon']):
        clue_boost += 0.05
    if suspect_name and suspect_name in game['liars']:
        clue_boost += 0.1
...
def rl_update(suspect, reward, game, suspect_name=None):
    current_value = suspect.get('value', suspect['probability'])
    future_value = 0.5 if suspect['lie_probability'] > 0.5 else 0.2
    if suspect_name and game['lie_counter'].get(suspect_name, 0) > 1:
        future_value += 0.3
...
# --- CSP Functions ---
def is_consistent(suspect_name, game):
    suspect = suspects[suspect_name]
    actual_or_fake_location = game['fake_locations'].get(suspect_name, suspect['location'])
    if actual_or_fake_location != suspects[game['killer_name']]['location']:
        return False
    has_strong_motive = any(m in suspect['motive'].lower() for m in ['revenge', 'jealousy', 'inheritance', 'financial', 'romantic'])
    is_liar = suspect_name in game['liars']
    return has_strong_motive or is_liar

def solve_with_csp(game):
    valid_candidates = [name for name in suspects if is_consistent(name, game)]
    return valid_candidates

# --- Dialogue Generation ---
def generate_suspect_dialogues(game):
    dialogues = {}
    for name, data in suspects.items():
        frontend_name = REVERSE_SUSPECT_MAPPING.get(name.lower(), name.lower())
        location_to_show = game['fake_locations'].get(name, data['location']).lower()
...
def start_game():
    ...
    for name in suspects:
        suspects[name]['probability'] = 0.5 if name == killer_name else suspects[name].get('probability', 0.3)
    ...
    if not liars:
        ...
        fake_locations[forced_liar] = random.choice(possible_locations)

    game_data = {
        'game_id': game_id,
        'killer_name': killer_name,
        'killer_weapon': killer_weapon,
        'fake_locations': fake_locations,
    ...
    try: 
        _db.games.insert_one(game_data)
        print(f"Game {game_id} saved to MongoDB")
    except Exception as e:
        return jsonify({
            'status': 'error',
            'message': f'Error occurred while starting the game: {str(e)}'
        }), 500
...
def round1_interview():
    ...
    game = _db.games.find_one({'game_id': game_id})
    ...
    suspect_dialogues = generate_suspect_dialogues(game)
...
def round2_alibis():
    ...
    for name in suspects:
        ...
        for target in seen:
            ...
            if name == game['killer_name'] or random.random() < suspects[name]['lie_probability']:
    ...
    return jsonify({
        'status': 'success',
        'alibi_claims': {REVERSE_SUSPECT_MAPPING.get(name.lower(), name): claims for name, claims in alibi_claims.items()}
    })
...
def round3_get_suggestion():
    ...
    for accuser, claims in game['alibi_claims'].items():
        weight = 1.5 if accuser not in game['liars'] else 1.0
        for target in claims.keys():
            mention_counts[target] += weight

    unchecked_suspects = [
        name for name in suspects.keys() if name not in game['checked_suspects']
    ]
    ...
    for name in unchecked_suspects:
        score = mention_counts[name] + suspects[name]['probability'] * 10
        if name == game['killer_name']:
            score += 2.0
        scores[name] = score
...
def round3_verify_alibi():
    ...
    if not suspect_name or suspect_name not in suspects:
        return jsonify({'status': 'error', 'message': 'Invalid suspect name'}), 400

    if suspect_name in game['checked_suspects']:
        return jsonify({'status': 'error', 'message': 'Suspect already verified'}), 400

    actual_location = suspects[suspect_name]['location']
    claimed_location = game['fake_locations'].get(suspect_name, actual_location)
    verification_result = {
    ...
    for accuser, claims in game['alibi_claims'].items():
        if suspect_name in claims:
            ...
            if is_correct:
                ...
                rl_update(suspects[accuser], reward, game, accuser)
                suspects[accuser]['probability'] = update_probability(
                    suspects[accuser], get_suspect_clues(suspects[accuser]), 
                    suspects[accuser]['lie_probability'], game, suspect_name=accuser
                )
            else:
                game['lie_counter'][accuser] += 1
                penalty = min(0.2, 0.05 + (game['lie_counter'][accuser] * 0.02))
                rl_update(suspects[accuser], -penalty, game, accuser)
                suspects[accuser]['probability'] = update_probability(
                    suspects[accuser], get_suspect_clues(suspects[accuser]), 
                    suspects[accuser]['lie_probability'], game, suspect_name=accuser
                )
    ...
    if not unchecked_suspects:
        ...
        max_mentions = max(mention_counts[name] for name in unchecked_suspects)
    ...
    # Update MongoDB
    checked = game['checked_suspects'].copy()
    checked.append(suspect_name)
    
    _db.games.update_one(
        {'game_id': game_id},
        {'$set': {
            'checked_suspects': checked,
            'lie_counter': game['lie_counter'],
            'updated_at': datetime.utcnow()
        }}
    )
...
def round4_final_deduction():
    game_id = request.args.get('game_id') or request.get_json().get('game_id')
    game = _db.games.find_one({'game_id': game_id})
    ...
    crime_scene = suspects[game['killer_name']]['location']
    for name, data in suspects.items():
        boost = evaluate_motive_opportunity(name, game['killer_weapon'], crime_scene, game['fake_locations'])
        if name == game['killer_name']:
            boost += 0.1
        suspects[name]['probability'] = min(1, suspects[name]['probability'] + boost)

    normalize_probabilities()
    valid_suspects_csp = solve_with_csp(game)

    scored_suspects = [
        (name, data['probability'] + (0.3 if name in valid_suspects_csp else 0) + (0.2 if name == game['killer_name'] else 0))
        for name, data in suspects.items()
    ]
    ...
    weapon_clue = random.choice(get_weapon_clues(game['killer_weapon']))

    # Update MongoDB
    _db.games.update_one(
        {'game_id': game_id},
        {'$set': {
            'valid_suspects_csp': valid_suspects_csp,
            'updated_at': datetime.utcnow()
        }}
    )
...
def make_guess():
    try:
        ...
        if backend_suspect == game['killer_name'] and backend_weapon == game['killer_weapon']:
            _db.games.update_one(
                {'game_id': game_id},
                {'$set': {'guess_count': guess_count, 'status': 'won', 'updated_at': datetime.utcnow()}}
            )
            return jsonify({
                'status': 'success',
        ...
        killer_clue = generate_killer_clue(game['killer_name'])
        weapon_clue = random.choice(get_weapon_clues(game['killer_weapon']))
        ...
        if remaining_tries <= 0:
            _db.games.update_one(
                {'game_id': game_id},
                {'$set': {'guess_count': guess_count, 'status': 'lost', 'updated_at': datetime.utcnow()}}
            )
            return jsonify({
                'status': 'game_over',
                'message': f'No tries left! Game over. The killer was {game["killer_name"]} with the {game["killer_weapon"]}.',
                'is_correct': False,
                'killer_clue': killer_clue,
                'weapon_clue': weapon_clue,
                'remaining_tries': 0
        ...
        _db.games.update_one(
            {'game_id': game_id},
            {'$set': {'guess_count': guess_count, 'updated_at': datetime.utcnow()}}
        )
...
@interview_bp.route('/mark_game_complete', methods=['POST'])
def mark_game_complete():
    ...
    return jsonify({'status': 'success', 'message': 'Game marked complete'})

@interview_bp.route('/mark_game_inactive', methods=['POST'])
def mark_game_inactive():
    ...
    return jsonify({'status': 'success', 'message': 'Game marked inactive'}) 
...
def game_status():
    ...
    return jsonify({
        ...
        'verified_suspects': game['checked_suspects'],
        'current_probabilities': {REVERSE_SUSPECT_MAPPING.get(name.lower(), name): round(data['probability'], 2) for name, data in suspects.items()}
    })
